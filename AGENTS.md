...
| **Slice plan** | Step-by-step checklist for the active slice. Checkbox state IS progress. | `docs/superpowers/plans/<latest>.md` (not in `done/`) |
| **Session log** | Journal of what happened in each work block. | `docs/superpowers/sessions/NNN-YYYY-MM-DD-<slug>.md` |
...
- **NEVER** write LICENSE/README boilerplate until requested.
...
- **NEVER** delete or rewrite files in `docs/superpowers/{specs,plans,sessions,research}/` without explicit instruction. They're history.
