...
- **D13 (new)**: SwiftPM package URL is **`https://github.com/argmaxinc/WhisperKit.git`** pinned to `from: "1.0.0"`. The MCP research initially suggested `argmaxinc/argmax-oss-swift.git@0.9.0`, but the upstream `gh release list` check during scaffolding showed both URLs resolve to the same `Package.swift` (package name `argmax-oss-swift`) and the current stable tag is **v1.0.0** (released 2026-05-01). `argmaxinc/WhisperKit` is kept as the package URL because it's the more discoverable historical name.
...
- **D5 amended (post-implementation, 2026-05-12)**: default model is `openai_whisper-base`, **not** `openai_whisper-tiny`. Empirical reason: WhisperKit's `tiny` misdetects Spanish audio as English in the conformance suite (`es_30s.wav`). `base` does too, but at least transcribes correctly with an explicit hint — see issue #125. Overridable via `PANOPS_WHISPERKIT_MODEL`. The sidecar validates the chosen variant against `WhisperKit.fetchAvailableModels(from: "argmaxinc/whisperkit-coreml")` at startup and falls back to the literal string if the list fetch fails (offline path).
- **D12 (new — partial)**: model cache directory is NOT explicitly overridden in slice 10. WhisperKit downloads under HuggingFace Hub conventions (typically `~/Documents/huggingface/`). Pre-downloading via `WhisperKit.download(variant:from:)` into `~/Library/Application Support/panops/models/whisperkit/` was scoped out of the walking skeleton; consider re-introducing in a future polish slice if multi-cache-dir confusion surfaces.
...
- **D15 (new)**: timing units differ — WhisperKit reports `start` / `end` in **seconds (Float)**, panops `Segment` uses `start_ms` / `end_ms` (u64). Convert at the adapter boundary (`Int(seconds * 1000.0)`). Additionally, WhisperKit emits occasional 1-ms rounding overlaps between adjacent segments; the adapter clamps `start_ms = max(rawStartMs, prevEnd)` to keep timestamps monotonic (required by the panops conformance suite).
- **D16 (new)**: platform requirement is macOS 14.0+ AND Xcode 16.0+ (CLT alone is not enough — WhisperKit ships CoreML model assets). CI's `macos-latest` runners have full Xcode pre-installed.
- **D17 (post-implementation, 2026-05-12)**: segment `text` from WhisperKit includes raw Whisper special tokens (`<|startoftranscript|><|en|><|transcribe|><|0.00|>` interleaved with transcribed words). The sidecar strips these via a `<\|[^|>]*\|>` regex before serializing the wire `Segment.text`.

Three of the original MCP findings (model variant, per-segment language, cache directory) were flagged "spike before locking" — the Task 3 spike verified the model-variant + language-field choices, and **D12** was relaxed from "explicit cache override" to "use WhisperKit's default cache" because the walking-skeleton didn't need the override.
...
| D6 | **Sidecar binary discovery: `PANOPS_ASR_SIDECAR_BIN` env var only in slice 10**. The Rust resolver checks the path is a regular executable file; otherwise falls back to `WhisperRsAsr`. The production `Bundle.main.bundleURL/Contents/Resources/panops-asr-mac` fallback lives in `apps/Panops/Sources/Panops/EngineProcess.swift`'s discovery code (mirrors slice 09's `PANOPS_ENGINE_BIN` pattern) but is **not yet wired into the Rust engine's adapter resolver** — that path activates in slice 12 (`.app` packaging + signing). | AGENTS.md forbids user-facing env vars but explicitly allows dev/CI escape hatches when flagged. The walking skeleton is env-var only; production `.app` lookup is slice 12's job. |
